...
const fetchUserBlogs = async (slug) => {
  const response = await axios.get(`https://bromine.vercel.app/api/blogs/${slug}`);
        ...
        <div className="container mx-auto py-12 px-4 lg:px-20">
          <Card className="shadow-lg">
            <CardHeader>
              ...
              <Link to={`/profile/${author_username}`}><p className="text-center text-gray-600 mb-4">By {author_username}</p></Link>
              <p className="text-center text-gray-600 mb-4"> {created_at.substring(0,10)}</p>
